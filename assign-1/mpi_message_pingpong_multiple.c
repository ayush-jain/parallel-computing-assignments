...
int main(int argc, char** argv) {
  ...
  if (world_size % 2 != 0) {
    fprintf(stderr, "World size must be even for %s\n", argv[0]);
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  ...
  // this time, pick a partner in the other half of the machine
  int partner_rank = (world_rank + world_size/2) % world_size;
  while (ping_pong_count < ping_pong_limit) {
    if (ping_pong_count % 2 == 0) {
    // in the even iteration, low processor sends and high receives
    ...
    // in the odd iteration, high processor sends and low receives
      if (world_rank > partner_rank) {
        MPI_Send(&data_buffer, message_size, MPI_FLOAT, partner_rank, 0, MPI_COMM_WORLD);
      // printf("%d sent and incremented ping_pong_count %d to %d\n",
      //       world_rank, ping_pong_count, partner_rank);
